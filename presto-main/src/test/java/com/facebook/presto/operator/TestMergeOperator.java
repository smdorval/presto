...
public class TestMergeOperator
{
    ...
    public void testMergeDifferentTypes()
            throws Exception
    {
        MergeOperator operator = createMergeOperator(ImmutableList.of(BIGINT, INTEGER), ImmutableList.of(1, 0), ImmutableList.of(1, 0), ImmutableList.of(DESC_NULLS_FIRST, ASC_NULLS_FIRST));
        operator.addSplit(createRemoteSplit(TASK_1_ID));
        operator.addSplit(createRemoteSplit(TASK_2_ID));
        operator.noMoreSplits();

        List<Page> task1Pages = rowPagesBuilder(ImmutableList.of(BIGINT, INTEGER))
                .row(0, null)
                .row(1, 4)
                .row(2, 3)
                .build();

        List<Page> task2Pages = rowPagesBuilder(ImmutableList.of(BIGINT, INTEGER))
                .row(null, 5)
                .row(2, 5)
                .row(4, 3)
                .build();
