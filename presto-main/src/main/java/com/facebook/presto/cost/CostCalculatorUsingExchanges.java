        ...
        implements CostCalculator
{
    ...
    @Override
    public PlanNodeCostEstimate calculateRootCost(Session session, Map<Symbol, Type> types, PlanNode planNode, Lookup lookup)
    {
            ...
            extends PlanVisitor<Void, PlanNodeCostEstimate>
    {
        ...
        private PlanNodeCostEstimate calculateJoinCost(PlanNode join, PlanNode probe, PlanNode build, boolean replicated)
        {
            ...
            Estimate cpuCost = probeStats.getOutputSizeInBytes()
                    .add(buildStats.getOutputSizeInBytes().map(size -> size * numberOfNodesMultiplier))
                    .add(outputStats.getOutputSizeInBytes());

            Estimate memoryCost = buildStats.getOutputSizeInBytes()
                    .map(size -> size * numberOfNodesMultiplier);
        ...
        {
            return lookup.getStats(session, types, node);
        }

        private PlanNodeCostEstimate getCost(PlanNode node)
        {
            return lookup.getCost(session, types, node);
        }
    }
    ...
    public static PlanNodeCostEstimate calculateExchangeCost(int numberOfNodes, PlanNodeStatsEstimate exchangeStats, ExchangeNode.Type type, ExchangeNode.Scope scope)
    {
        ...
        switch (type) {
            ...
            case REPARTITION:
                ...
                break;
            case REPLICATE:
                network = exchangeStats.getOutputSizeInBytes().map(value -> value * numberOfNodes);
                break;
            default:
